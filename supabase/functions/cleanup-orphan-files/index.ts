...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.error('[cleanup-orphan-files] No authorization header provided');
      return new Response(
    ...
    if (userError || !user) {
      console.error('[cleanup-orphan-files] Invalid user token:', userError);
      return new Response(
    ...
    console.log(`[cleanup-orphan-files] Authenticated user: ${user.email}`);
